...
# Get a list of all unique cell types in metadata and save to a file
unique_celltypes <- unique(metadata$celltype)
write.csv(unique_celltypes, 'unique_celltypes.csv', row.names = FALSE)
print("Unique cell types saved to 'unique_celltypes.csv'")

# Optionally, display the unique cell types
...
enhancer.pairs <- read.csv('/Users/huajingru/Desktop/Fall 2024/Capstone/CDS-2024-Fall-Capstone/data_processing/00_pairs/00_pairs1.csv')
...
for (i in 1:nrow(enhancer.pairs)) {
  ...
  print(paste('enhancer.1.atac:', enhancer.1.atac))
  print(paste('enhancer.2.atac:', enhancer.2.atac))
  print(paste('gene.rna:', gene.rna))
}
...
write.csv(results, 'model_results_test0.csv', row.names = FALSE)
